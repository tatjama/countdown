// ======== Initialization ========
  ...
  // ======== Modal Open / Close ========  
  function openSettings() { overlayEl.classList.add('active'); }
  function closeSettings() { overlayEl.classList.remove('active'); }
  ...
  title.split(' ').map(el => titleEl.innerHTML += `<span>${el}</span>`);
  document.body.style.backgroundColor = theme==='dark'? '#000' : theme==='light'? '#fff' : 'transparent';
  document.body.style.backgroundImage = theme==='transparent' &&  `linear-gradient(to bottom, rgba(0,0,0,0) 0%, rgba(0,0,0,1) 100%)`;
  document.body.style.color = theme==='dark'? '#fff' : theme==='light'? '#000' : '#fff';
